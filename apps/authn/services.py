...
def _otp_cache_key(phone: str) -> str:
    return f"otp:login:{normalize_phone_e164(phone)}"
...
def _ensure_driver_profile(user) -> None:
...
def verify_otp_and_issue_tokens(phone: str, otp: str, role: str = "customer") -> dict:
    ...
    existing, _canonical = find_user_by_phone(user_model, phone)
    if existing:
        user = existing
    else:
